...
# Converting orderdate column to datetime and create a Week column (starting Monday)
...
# Cleaning the websource"column and campaign column.
...
# (D) Promotion Analysis: Top 10 Promotions per Week
print("=== (D) Promotion Analysis: Top 10 Promotions per Week (Excluding 'NoPromotion') ===")

# Creating unified promotion labels.
...
# Aggregating weekly metrics by promotion.
...
# Part D2: Selecting Top 10 Promotions (Excluding 'NoPromotion') per Week

# Excluding baseline rows.
...
# Computing the number of active days for each promotion per week.
daily_counts = website_sales_df.groupby(['Week', 'promo_clean']).agg(
    days_active = ('order_ts', 'nunique')
).reset_index()

# Merging the daily counts into the aggregated weekly metrics.
...
# Part D4: Creating a Tidy Table for Top 10 Promotions with Uplift Metrics

# Restricting to only the top 10 promotions per week
top10_keys = top10_weekly_promos[['Week', 'promo_clean']].drop_duplicates()

# Merging to retrieve the daily averages for these top 10 promotions.
top10_daily = pd.merge(top10_keys, agg_metrics, on=['Week', 'promo_clean'], how='left')

# Retrieving the baseline ("NoPromotion") daily averages for each week.
...
# Merging baseline averages with the top 10 promotions (by Week).
top10_daily = pd.merge(top10_daily, baseline_data, on="Week", how="left")

# Computing uplift metrics relative to the baseline daily averages.
...
# Creating the final tidy table containing only desired columns.
...
#Writing the Tidy Table to PostgreSQL
...
# First, we sort the detailed weekly author metrics by Week and then by Total_Profit (descending).
author_weekly_ranked = author_analysis.sort_values(['Week', 'Total_Profit'], ascending=[True, False]).copy()

# Computing a rank per week.
author_weekly_ranked['rank'] = author_weekly_ranked.groupby('Week').cumcount() + 1

# Filtering to keep only the top 10 authors for each week.
...
# Grouping by week, strategy base, and test status
...
# PART D: GOOGLE ANALYTICS WEEKLY AGGREGATIONS 
...
# Replacing null/missing landing_page values with "unspecified".
...
# Sorting by Week and descending purchase revenue, then pick the top 10 rows for each week.
...
# Creating a Week column starting from Monday.
...
# Creating a synthetic primary key in both tables based on: date, campaign_id, adset_id, ad_id
...
# Merging insights and pre-filtered actions
...
# Converting date to datetime, and derive Weekly column (Monday as start)
...
# Aggregating the Facebook metrics by week:
...
# Computing advanced metrics at the campaign level.
