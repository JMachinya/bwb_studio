...
# 4) HELPER FUNCTIONS FOR REGISTRATION & LOGIN (unchanged)
...
def check_credentials(username: str, password: str) -> bool:
    ...
    with engine.connect() as conn:
        result = conn.execute(select_query, {"username": username, "password": password}).fetchone()
    return result is not None

# 5) LOGIN / REGISTRATION UI (ONLY IF NOT LOGGED IN)
    ...
    if auth_choice == "Register":
        ...
        if st.button("Register", key="register_button"):
            if reg_password != reg_confirm:
                ...
                st.info("Registration complete. Please switch to Login.")
    else:
        ...
        if st.button("Login", key="login_button"):
            if check_credentials(login_username, login_password):
                ...
                st.rerun()  # Force rerun to display dashboard
            else:
                st.error("Invalid username or password.")
    st.stop()

# 6) DASHBOARD CODE (VISIBLE ONLY IF LOGGED IN)
